...
我们新增了灵活的AI模型配置功能，让系统能够适应不同场景下的AI需求：

- **多模型支持**：现已支持OpenAI、Anthropic Claude、阿里云通义千问、百度文心一言及自定义模型
- **API灵活配置**：可自行设置API密钥、基础URL和各种模型参数
- **多模态能力**：针对不同模型，可配置其图像识别等多模态分析能力
- **故障转移机制**：当主要模型不可用时，系统可自动切换到备用模型
- **参数微调**：提供温度、上下文长度等高级参数配置，满足不同应用场景

访问路径：通过侧边栏的"系统设置"进入，选择"模型设置"标签页即可管理所有AI模型配置。
