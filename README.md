# Argus AI (阿格斯之眼) - 智能订单管理与分析系统

一个全链路全时态智能监测平台，集成了 n8n 自动化引擎与多模态 AI，采用苹果设计风格的用户界面。

---

## ✅ 已上线功能：N8N 工作流驱动的 AI 智能分析 (Argus 智脑)

> **状态**: 已上线 | **核心实现**: n8n Workflow + Supabase Realtime

此功能已全面集成 n8n 自动化引擎，为异常订单提供 100% 真实的深度分析。系统不再使用本地模拟逻辑，而是通过触发真实的 n8n 工作流，实时检索日志、关联数据库信息，并将分析过程与结果实时同步至前端。
...
- [x] **Phase 2: 工作流构建** (已完成)
    ...
    - 对接真实数据库节点。
- [x] **Phase 3: 实时可视化** (已完成)
    - 利用 Supabase Realtime 实现思考过程的秒级同步。
    - 前端动态渲染"思考过程"步骤。
- [x] **Phase 4: 移除本地模拟** (已完成)
    - 彻底删除 `AIService` 中的模拟逻辑。
    - 确保 UI 仅展示真实数据，优化错误处理机制。
...
本系统在开发环境下（`process.env.NODE_ENV === 'development'`）使用高度真实的模拟数据，以支持开发、演示及AI分析功能的深度测试。

### 模拟数据特性
- **100条订单数据**：包含15%的异常订单（exception），以及待处理（pending）、处理中（processing）和已完成（completed）的各种状态。
- **8种异常类型**：除了基础的库存不匹配、质量问题等，新增了支付异常（payment_issue）、系统错误（system_error）和重复订单（duplicate_order）。
- **30+条库存数据**：分布在全国8个核心仓库。
- **20+条AI洞察**：覆盖库存管理、销售预测、供应链、效率及异常识别。
- **12个月历史概览**：包含入库/出库趋势、营收及利润分析。

### 如何使用
点击任意标记为红色的异常订单，通过「AI查看」按钮即可触发AI辅助排查流程。系统会自动根据订单的细节数据（如异常描述、产品信息、客户反馈等）整合模拟的日志和数据库背景，生成针对性的解决方案。
