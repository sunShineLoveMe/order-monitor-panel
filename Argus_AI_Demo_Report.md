...
### 3.2 2025 全时态活动监测 (Activity Heatmap)
系统引入了类 GitHub 贡献矩阵的 **52周全时态活动热力图**，作为“总览”面板的核心决策支持工具。

![2025 全链路活动趋势监测](docs/assets/argus_overview_final_report_1766368648939.png)
*图3：集成在总览面板底部的 2025 年度全链路活动趋势图*

#### 该特征的核心业务意义：
1.  **全局健康扫描**: 决策者可一眼横跨 2025 全年 365 天，实时感知仓储、出入库、销售活动的“数字化脉搏”。
2.  **异常取证支持**: 当 AI 检测到异常（如利润波动）时，热力图提供的历史强度数据是 AI 进行“根因分析 (RCA)”的关键视觉证据。
3.  **预测策略对齐**: 通过对比历史热度与 AI 生成的销售预测图，管理层可更直观地验证补货策略与市场活跃度是否匹配。
4.  **沉浸式指挥体验**: 采用 Cyber-Emerald (赛博翡翠) 配色调优，不仅在视觉上极具“极客感”，更通过悬浮提示 (Tooltips) 提供了精细到日度的“数字显微镜”功能。
